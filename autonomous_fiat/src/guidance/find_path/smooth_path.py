...
def increase_points(smooth_path, num_points):
    x = [point[0] for point in smooth_path]
    y = [point[1] for point in smooth_path]
    fx = interp1d(range(len(x)), x)
    fy = interp1d(range(len(y)), y)
    new_x = fx(np.linspace(0, len(x)-1, num_points))
    new_y = fy(np.linspace(0, len(y)-1, num_points))
...
def smoothPathMain(path, image_name=FILE_NAME, car_width=CAR_WIDTH, car_length=CAR_LENGTH, weight_data=WEIGHT_DATA, weight_smooth=WEIGHT_SMOOTH, tolerance=TOLERANCE, visualize=VISUALIZE):
    ...
    finalPath = increase_points(smoothPath, len(path))
    finalPath = np.array(finalPath) 

    if visualize:
        visualize_path(path, my_car, map, finalPath)
    
    finalPath[:, [0, 1]] = finalPath[:, [1, 0]]
