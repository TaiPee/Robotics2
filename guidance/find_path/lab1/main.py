# serial port to communicate with robot
SERIAL_PORT_NAME = 'COM11'

# list images available
IMAGES = ['test_draw_1.png', 'test_draw_2.png']
...
# index of image to draw inside IMAGES list (need to have been processed before or in the same run)
ROBOT_IM_NUM = 0
ROBOT_IMG_NAME = IMAGES[ROBOT_IM_NUM] # set to None to not draw anything 
...
def main ():
    ...
    for filename in PROCESS_IMG_NAMES:
        ...
        # downsample path 
        clusters = img.downSampleClusters(clusters, MAX_POINTS)
    ...
    ########## MAKE ROBOT DRAW SPECIFIED PREPROCESSED IMAGE ##########
    if ROBOT_IMG_NAME is not None:
        rb.robot_main(ROBOT_IMG_NAME.split('/')[-1].split('.')[0] + '.txt', SERIAL_PORT_NAME)
